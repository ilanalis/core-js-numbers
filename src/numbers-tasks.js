...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleBetweenVectors = Math.acos(scalarProduct / (vector1 * vector2));
...
function toNumber(value, def) {
  ...
  return Number.isNaN(toNumberValue) ? def : toNumberValue;
}
...
function getSumOfDigits(num) {
  const stringOfNumbers = String(num);
  let sum = 0;
  for (let i = 0; i < stringOfNumbers.length; i += 1) {
    sum += +stringOfNumbers[i];
  }
  return sum;
}
...
function isPowerOfTwo(num) {
  let numberToThePower = 2;
  for (let i = 1; numberToThePower < num; i += 1) {
    numberToThePower = 2 ** i;
  }
  return numberToThePower === num;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.abs(x1 + x2 + x3).toFixed(1);
}
